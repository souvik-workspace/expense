...
<body id="page-top">
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-body" style="min-height:200px;">
                                    <div class="container">
                                        <form action="{{url('editExpenseSubmit/'.$expense->id)}}" method="POST">
                                            @csrf  
                                            <div class="form-group">
                                                <label for="expense_date">Expense Date:</label>
                                                <input type="datetime-local" name="expense_date" class="form-control" id="expense_date" style="color: red" value="<?php if($expense->expense_date !=""){echo date("Y-m-d\TH:i", strtotime($expense->expense_date)); }else{ echo date("Y-m-d\TH:i", strtotime("now"));}?>"  placeholder="Enter expense date">
                                            ...
                                            <option value="">Select Group</option>
                                                @if(!empty($groups))
                                                    @foreach($groups as $row)
                                                        <?php if($row->id == $expense->group_id){ $selected = "Selected";}else{ $selected = "";} ?>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" id="item_id_ajax" value="{{$expense->item_id}}">                                         
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="number" name="amount" class="form-control" id="amount" value="{{$expense->amount}}" placeholder="Enter expenses amount">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="pay_method" class="form-control" id="pay_method"> 
                                                    ...
                                                    <option value="bank" <?php if($expense->pay_method =="bank"){ echo "Selected";}?>>Bank</option>                                           
                                                </select>    
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="note" class="form-control" id="note" value="{{$expense->note}}" placeholder="Enter any note">
    ...
    <script>
        $(document).ready(function () {
            ...
            // onload auto filling item 
            var group_id = $('#group_id').find(":selected").val();
            var item_id  = $('#item_id_ajax').val();            
            $("#item_id").html('');
                $.ajax({
                    url: '/getItem/'+group_id,
                    type: "GET",
                    ...
                    success: function (result) {                        
                        $('#item_id').html('<option value="">-- Select Item --</option>');
                        $.each(result, function (key, value) {
                            if(value.id==item_id){ var selected ="Selected";}else{ var selected ="";}
                            $("#item_id").append('<option value="' + value.id + '" '+ selected +'>' + value.item_name + '</option>');
                        });                        
                    }
            });
